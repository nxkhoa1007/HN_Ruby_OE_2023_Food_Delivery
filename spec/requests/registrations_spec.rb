...
RSpec.describe Users::RegistrationsController, type: :controller do
  ...
  describe "#after_inactive_sign_up_path_for" do
    it "returns the path to the new user registration page" do
      controller = Users::RegistrationsController.new
      result = controller.after_inactive_sign_up_path_for(user)
      expect(result).to eq(routes.url_helpers.new_user_registration_path(user))
    end
  end
end
